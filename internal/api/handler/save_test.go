...
func TestGetSaveSlots(t *testing.T) {
	...
	t.Run("获取存档槽位列表", func(t *testing.T) {
		...
		w := env.doRequest("GET", "/api/v1/sessions/"+sessionID+"/saves", nil)
...
func TestSaveGame(t *testing.T) {
	...
	t.Run("保存游戏到槽位1", func(t *testing.T) {
		...
		w := env.doRequest("POST", "/api/v1/sessions/"+sessionID+"/saves/1", nil)
		...
		if data["success"] != true {
			t.Errorf("保存应该成功, message: %v", data["message"])
		}
	})
	...
	t.Run("无效槽位号", func(t *testing.T) {
		...
		// 槽位只有1-3
		w := env.doRequest("POST", "/api/v1/sessions/"+sessionID+"/saves/0", nil)

		if w.Code != http.StatusBadRequest {
			t.Errorf("期望状态码 %d, 得到 %d", http.StatusBadRequest, w.Code)
		}

		w = env.doRequest("POST", "/api/v1/sessions/"+sessionID+"/saves/4", nil)
...
func TestLoadGame(t *testing.T) {
	...
	t.Run("保存后加载游戏", func(t *testing.T) {
		...
		// 保存游戏
		env.doRequest("POST", "/api/v1/sessions/"+sessionID+"/saves/1", nil)
		...
		// 加载游戏
		w := env.doRequest("POST", "/api/v1/sessions/"+sessionID+"/saves/1/load", nil)
		...
		if data["success"] != true {
			t.Errorf("加载应该成功, message: %v", data["message"])
		}
	...
	t.Run("加载不存在的存档", func(t *testing.T) {
		...
		// 使用槽位3（假设为空）
		w := env.doRequest("POST", "/api/v1/sessions/"+sessionID+"/saves/3/load", nil)
