...
func TestGetCommodities(t *testing.T) {
	...
	t.Run("获取商品列表", func(t *testing.T) {
		...
		w := env.doRequest("GET", "/api/v2/sessions/"+sessionID+"/shop/commodities", nil)
...
func TestBuyCommodity(t *testing.T) {
	...
	t.Run("购买商品成功", func(t *testing.T) {
		...
		// 购买面包（价格15元）
		w := env.doRequest("POST", "/api/v2/sessions/"+sessionID+"/shop/buy/面包", nil)
		...
		resp, _ := parseResponse(w)
		if !resp.Success {
			t.Fatalf("购买应该成功, error: %+v", resp.Error)
		}
	...
	t.Run("高余额购买昂贵商品", func(t *testing.T) {
		sessionID := env.createTestSession()

		boost := env.setTestMoney(sessionID, 1000, 0)
		if boost.Code != http.StatusOK {
			t.Fatalf("期望状态码 %d, 得到 %d", http.StatusOK, boost.Code)
		}
		...
		// 购买牛排（价格150元）
		w := env.doRequest("POST", "/api/v2/sessions/"+sessionID+"/shop/buy/牛排", nil)
		resp, _ := parseResponse(w)
		if !resp.Success {
			t.Fatalf("高余额时购买应该成功, error: %+v", resp.Error)
		}
	})
	...
	t.Run("购买不存在的商品", func(t *testing.T) {
		...
		w := env.doRequest("POST", "/api/v2/sessions/"+sessionID+"/shop/buy/不存在的商品", nil)
	...
	t.Run("余额不足无法购买", func(t *testing.T) {
		...
		// 多次购买直到余额不足
		for i := 0; i < 10; i++ {
			env.doRequest("POST", "/api/v2/sessions/"+sessionID+"/shop/buy/面包", nil)
		}
