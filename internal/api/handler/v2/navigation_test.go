...
func TestNavigate(t *testing.T) {
	...
	t.Run("导航到超市成功", func(t *testing.T) {
		...
		w := env.doRequest("POST", "/api/v2/sessions/"+sessionID+"/navigate/1", nil)
	...
	t.Run("导航到银行成功", func(t *testing.T) {
		...
		w := env.doRequest("POST", "/api/v2/sessions/"+sessionID+"/navigate/2", nil)
	...
	t.Run("导航到医院成功", func(t *testing.T) {
		...
		w := env.doRequest("POST", "/api/v2/sessions/"+sessionID+"/navigate/4", nil)
	...
	t.Run("导航到无效位置", func(t *testing.T) {
		...
		w := env.doRequest("POST", "/api/v2/sessions/"+sessionID+"/navigate/999", nil)
	...
	t.Run("无效Session返回404", func(t *testing.T) {
		w := env.doRequest("POST", "/api/v2/sessions/invalid-id/navigate/1", nil)
