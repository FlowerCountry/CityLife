...
func TestGetActions(t *testing.T) {
	...
	t.Run("获取可用行动列表", func(t *testing.T) {
		...
		w := env.doRequest("GET", "/api/v1/sessions/"+sessionID+"/actions", nil)
	...
	t.Run("无效Session返回404", func(t *testing.T) {
		w := env.doRequest("GET", "/api/v1/sessions/invalid-id/actions", nil)
...
func TestExecuteAction(t *testing.T) {
	...
	t.Run("执行查看钱包行动", func(t *testing.T) {
		...
		w := env.doRequest("POST", "/api/v1/sessions/"+sessionID+"/actions", map[string]string{
			"action_id": "check_cash",
		})
